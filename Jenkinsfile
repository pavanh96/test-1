...
sh 'git --version'
 ...
 stage('Docker Image to ECR'){
  steps{
   ...
   sh 'docker push 230226440659.dkr.ecr.us-east-1.amazonaws.com/jenkins-test:latest'
  }
 }
 stage('Docker Deploy'){
  steps{
   sh ' docker run -itd -p 92:92 docker push 230226440659.dkr.ecr.us-east-1.amazonaws.com/jenkins-test:latest'
   sh ' docker ps'
  }
 }
 stage('Docker Image'){
